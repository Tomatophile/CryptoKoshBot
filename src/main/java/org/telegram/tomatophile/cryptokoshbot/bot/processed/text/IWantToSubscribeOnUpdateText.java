...
public class IWantToSubscribeOnUpdateText implements Text {
    @Getter
    private final String text = "Хочу подписаться на обновление курса криптовалюты.";

    private final ReplyService replyService;

    @Override
    public List<PartialBotApiMethod<Message>> process(Update update) {
        return List.of(replyService.getTextMessage(update.getMessage().getChatId().toString(), "Для этого напиши мне команду : /subscribeUpdate <FIGI валюты>"));
    }
}
